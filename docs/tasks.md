...
| Criteri completamento | Definita struttura stabile del CSV unico di export/import; esportazione include ingredienti, ricette, relazioni ricetta-ingredienti, piani, slot piano, lingua e tema; import valida formato, versioning schema, campi obbligatori, duplicati, riferimenti tra ricette/ingredienti e piani/ricette; import gestisce errori senza corrompere i dati esistenti; privacy e sicurezza rispettate; `IMPORT_EXPORT.md` spiega header, `record_type`, colonne, valori ammessi, esempi minimi e regole di compatibilita. In tab `Altro` esiste una sezione `Import/export` dedicata: import permette di scegliere un CSV dalle cartelle del telefono tramite picker di sistema, mostra il file selezionato e richiede conferma esplicita prima di modificare il DB; export genera/condivide un CSV unico. Il flusso import mostra avanzamento a step con spinner sullo step attivo e icone successo/errore sugli step completati: selezione file, lettura CSV, validazione formato/schema, validazione preferenze, ingredienti, ricette, relazioni, piani/slot, riepilogo, popolamento DB, aggiornamento app. Se una validazione fallisce, DB e stato app restano invariati e la UI mostra lo step fallito con messaggio comprensibile. Dopo import/export completati, la sezione mostra ultimo import/export con data e ora. |
| Verifica/test | `IMPORT_EXPORT.md` creato con specifica CSV unico tipizzato; unit/integration test futuri per serializer/parser, validazioni, riferimenti mancanti, duplicati e preferenze; test repository/persistence se import scrive DB; UI test futuri per sezione `Import/export`, file selezionato, conferma import, step progress, errore validazione, successo e timestamp ultimo import/export; `npm run typecheck` OK; `npm run test -- --runInBand` OK; smoke manuale APK: selezione CSV da cartelle telefono, import valido, import invalido senza modifica DB, export e condivisione CSV; file generato apribile in Excel/LibreOffice come CSV UTF-8. |
