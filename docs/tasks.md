...
| Verifica/test | Sviluppo completato lato codice: domain slot multi-ricetta, no duplicati, rimozione singola/svuotamento e compatibilita tag; mapper repository con normalizzazione legacy `recipeId` -> `recipeIds[]`; appModel per aggiunta, rimozione singola, conteggio riferimenti e cancellazione ricetta referenziata; generatore random mantiene almeno una ricetta compatibile per slot; import/export CSV con lista `recipe_id` separata da `;` e compatibilita legacy; UI Piano e Oggi aggiornate per mostrare piu ricette nello stesso pasto; allineamento visivo dei bottoni `+` e cestino nello slot Piano corretto; `docs/requirements.md`, `docs/architecture.md`, `IMPORT_EXPORT.md` e README allineati. `npm run typecheck` OK; `npm run test -- --runInBand` OK, 66 test passati con warning `act(...)` noti; verifica UI mirata dopo allineamento bottoni OK con `npm run test -- src/test/__tests__/ui.test.tsx --runInBand`; `npm run build:android` OK; `npm run build:web` OK. Pendente smoke manuale APK mirato: creazione piano con pranzo composto da piu ricette, verifica allineamento azioni nello slot, riapertura app, export CSV e reimport. |
